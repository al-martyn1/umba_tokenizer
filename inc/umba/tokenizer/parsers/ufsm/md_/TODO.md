...
- [ ] При рисовании переходов в самих себя будем использовать форму треугольника с закруглёнными углами, стоящего на вершине
      ...
      UPD: не треугольник, а домик с треугольной крышеей, стоящий на крыше вверх головой (если мы говорим об отображении 
      перехода сверху).
...
- [ ] Shape состояний. Пока не будем экспериментировать, будем рисовать состояния в виде окружностей:
  - окружность - обычное состояние;
  - двойная окружность - конечное состояние;
  - окружность с толстым контуром линией (аналогично предыдущему, только контур между окружностями закращен) - начальное состояние. 
    Или, может, тройной контур? Чтобы чернила попусту не расходовать;
  - окружность, сверху и снизу хорды - проходное transition-состояние;

- [ ] Метки состояний. Метки рисуем, выравнивая по центру как горизонтально, так и вертикально.
  Следить за тем, чтобы они не выезжали за пределы окружностей - не будем. Пусть автор даёт состояниям короткие имена, у нас есть 
  описания, там можно раскрыть абревиатуру, и кратко описать состояние. Например: состояние - "WS", описание - 
  "WAIT_START - ожидание старта последовательности", "RB" - "READ_BYTES - Чтение байт", "RFB" - "READ_FIRST_BYTE - 
  чтение первого байта".


### Легенда

- [ ] Легенду можем выводить вместе с графом на одном рисунке, или можно выводить отдельно граф и отдельно легенду. 
      Также легенду можно выводить текстом (markdown).

- [ ] Состав легенды:
  - легенда состояний;
  - легенда действий (actions)
  - легенду предикатов

- [ ] Вывод легенды: можно указывать, что выводить в легенде. Таким образом, имя возможность вывода легенды отдельно, 
      и управляя составом легенды, её можно разделить на несколько частей. И, например, для каждой части при вставке 
      в документ сделать свой подзаголовок.

- [ ] Легенда состояний. Тут просто: имя (название) состояния - описание.

- [ ] Легенда действий (actions). Мы хотим, чтобы на графе у нас было мало информации, чтобы его не перегружать визуально, 
      и иметь возможность его отобразить графически достаточно компактно. Для этого мы задаём составные действия,
      и в действиях состояния (по входу/выходу), и в действиях переходов мы указываем составные действия.
      В легенде действий легенду можно выводить двумя способами - так, как это задано в описании автомата, или раскрывая
      полностью до всех `extern` действий. В первом случае мы игнорируем действия, которые не используются на графе, 
      во втором случае мы игнорируем действия, которые не используются на графе, и которые не входят в другие действия.

- [ ] Легенда предикатов - аналогично легенде действий.
