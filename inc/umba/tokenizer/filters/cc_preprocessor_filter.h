/*! \file
    \brief Фильтр, вставляющий маркеры (без полезной нагрузки) препроцессора C/C++. Переключает режим угловых скобок, чтобы их содержимое парсилось как строка. Переключает режим символа '#'.
 */
#pragma once
